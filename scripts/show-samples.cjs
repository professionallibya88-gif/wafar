    ...
    if (e.isDirectory()) {
      ...
      lines.forEach((line, idx) => {
        ...
        if (hasSuspect && hasArabic) {
          samples.push({ path: prefix + e.name, line: idx + 1, text: line.trim().slice(0, 120) });
        }
      });
    }
  }
...
console.log('Found ' + samples.length + ' suspect lines');
samples.slice(0, 60).forEach(f => console.log(f.path + ':' + f.line + ' | ' + f.text));
