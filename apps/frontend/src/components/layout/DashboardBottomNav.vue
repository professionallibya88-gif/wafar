  ...
  >
    ...
    <div class="relative grid grid-cols-5 gap-1.5 p-2 pb-safe z-10">
      <template v-for="item in items" :key="item.path">
        <!-- الزر الأوسط (رفع ملف) -->
        <div v-if="item.path === '/upload'" class="relative flex flex-col items-center justify-end pb-1.5 xs:pb-2 px-1 rounded-2xl min-h-[60px] xs:min-h-[64px]">
          <router-link
            :to="item.path"
          ...
          <span 
            class="text-xs font-medium transition-colors relative z-10 pt-4" 
